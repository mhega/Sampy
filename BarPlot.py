class BarPlot:
    ...
    def appendBarPlot(tupList, dataColumnIndex): ## ALWAYS call this function with tuple list that is sorted (descending) by the data column which is to be plotted
        ...
        maxValueLength = len(str(tupList[0][dataColumnIndex]))
        ...
        if int(tupList[0][dataColumnIndex]) > 60:
            _scale = 1
        else:
            _scale = 2
        ...
        #return [tup+(''.join([u"\u2584" for i in range(int(int(tup[dataColumnIndex])/divident))]),) for tup in tupList]
        #Unicode "LOWER_HALF_BLOCK" did not work well on Linux
        
        return [tup+(''.join(['*' for i in range(int(int(tup[dataColumnIndex])/divident))]),) for tup in tupList]
